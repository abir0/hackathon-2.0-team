...
def collect_data():
    ...
    # users = Video.objects.order_by().values('author_usernsame', flat=True).distinct()

    # for username in users:
    #     user_data = asyncio.run(tiktok_scraper.scrape_tiktok_user(username=username))
    #     for _user in user_data:
    #         _u = Video.objects.update_or_create(**_user)
